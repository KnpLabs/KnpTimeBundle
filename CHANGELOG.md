## [v1.9.1](https://github.com/symfony/maker-bundle/releases/tag/v1.9.1)

*April 19th, 2019*

### UNKNOWN

- [#117](https://github.com/knplabs/KnpTimeBundle/pull/117) - Use transChoice for Symfony <4.2 - *@cb8*

## [v1.3](https://github.com/symfony/maker-bundle/releases/tag/v1.3)

*December 12th, 2013*

## [v1.2](https://github.com/symfony/maker-bundle/releases/tag/v1.2)

*November 15th, 2013*

## [v1.16.0](https://github.com/symfony/maker-bundle/releases/tag/v1.16.0)

*March 2nd, 2021*

### Minor

- [#155](https://github.com/knplabs/KnpTimeBundle/pull/155) - WIP move CI to github actions - *@jrushlow*

### UNKNOWN

- [#156](https://github.com/knplabs/KnpTimeBundle/pull/156) - change CI and bump supported versions - *@jrushlow*

## [v1.15.0](https://github.com/symfony/maker-bundle/releases/tag/v1.15.0)

*January 15th, 2021*

### Minor

- [#143](https://github.com/knplabs/KnpTimeBundle/pull/143) - Update README.md - *@ThomasLandauer*

### UNKNOWN

- [#147](https://github.com/knplabs/KnpTimeBundle/pull/147) - Add compatibility for PHP 8.0 - *@m-vo*
- [#153](https://github.com/knplabs/KnpTimeBundle/pull/153) - implement PHP's type system - *@jrushlow*
- [#152](https://github.com/knplabs/KnpTimeBundle/pull/152) - added the locale parameter - *@jpgiroux*
- [#144](https://github.com/knplabs/KnpTimeBundle/pull/144) - Update composer.json - *@ThomasLandauer*
- [#150](https://github.com/knplabs/KnpTimeBundle/pull/150) - Added Norwegian bokmål and Norwegian nynorsk translations - *@jon-harald*
- [#154](https://github.com/knplabs/KnpTimeBundle/pull/154) - updating README for locale override - *@weaverryan*
- [#149](https://github.com/knplabs/KnpTimeBundle/pull/149) - Update/doc - *@muchafm*

## [v1.13.0](https://github.com/symfony/maker-bundle/releases/tag/v1.13.0)

*August 14th, 2020*

### UNKNOWN

- [#105](https://github.com/knplabs/KnpTimeBundle/pull/105) - Added bundle usage docs in symfony controllers - *@EliuTimana*
- [#141](https://github.com/knplabs/KnpTimeBundle/pull/141) - Remove symfony/templating as a hard dependency - *@kbond*

## [v1.12.0](https://github.com/symfony/maker-bundle/releases/tag/v1.12.0)

*July 7th, 2020*

### UNKNOWN

- [#138](https://github.com/knplabs/KnpTimeBundle/pull/138) - Migrate to PSR-4 - *@mhor*
- [#109](https://github.com/knplabs/KnpTimeBundle/pull/109) - Fixed Serbian translation - *@ljubar*
- [#139](https://github.com/knplabs/KnpTimeBundle/pull/139) - Fix tests - *@weaverryan*
- [#136](https://github.com/knplabs/KnpTimeBundle/pull/136) - Add thai translations - *@Seb33300*
- [#132](https://github.com/knplabs/KnpTimeBundle/pull/132) - Fixed phpunit tests path - *@zerkms*
- [#137](https://github.com/knplabs/KnpTimeBundle/pull/137) - Add bg translation - *@ggabrovski*

## [v1.11.0](https://github.com/symfony/maker-bundle/releases/tag/v1.11.0)

*December 30th, 2019*

### UNKNOWN

- [#124](https://github.com/knplabs/KnpTimeBundle/pull/124) - Fix typo in README - *@svenluijten*
- [#127](https://github.com/knplabs/KnpTimeBundle/pull/127) - use namespaced twig classes - *@zaggi-iwnl*
- [#121](https://github.com/knplabs/KnpTimeBundle/pull/121) - Allow Symfony 5 - *@tacman*
- [#129](https://github.com/knplabs/KnpTimeBundle/pull/129) - bumping php & symfony version dependencies and making code updates - *@weaverryan*
- [#123](https://github.com/knplabs/KnpTimeBundle/pull/123) - removing support for most obsolete versions - *@weaverryan*

## [v1.1](https://github.com/symfony/maker-bundle/releases/tag/v1.1)

*January 11th, 2013*

## [v1.0](https://github.com/symfony/maker-bundle/releases/tag/v1.0)

*January 11th, 2013*

## [1.9.0](https://github.com/symfony/maker-bundle/releases/tag/1.9.0)

*December 19th, 2018*

### UNKNOWN

- [#111](https://github.com/knplabs/KnpTimeBundle/pull/111) - Fix symfony 4.2 transChoice deprecation - *@dirkkredler*

## [1.8.0](https://github.com/symfony/maker-bundle/releases/tag/1.8.0)

*December 26th, 2017*

### UNKNOWN

- [#103](https://github.com/knplabs/KnpTimeBundle/pull/103) - [#SymfonyConHackday2017] SF4 compatibility update - *@michalkurzeja*

## [1.7.5](https://github.com/symfony/maker-bundle/releases/tag/1.7.5)

*October 27th, 2017*

### UNKNOWN

- [#99](https://github.com/knplabs/KnpTimeBundle/pull/99) - Added a maintainer in the README.md - *@NicolasNSSM*
- [#101](https://github.com/knplabs/KnpTimeBundle/pull/101) - Fix romanian translation of minute (singular) - *@sergiu-popa*

## [1.7.4](https://github.com/symfony/maker-bundle/releases/tag/1.7.4)

*July 12th, 2017*

### UNKNOWN

- [#95](https://github.com/knplabs/KnpTimeBundle/pull/95) - Add .gitattributes - *@garak*
- [#97](https://github.com/knplabs/KnpTimeBundle/pull/97) - Add Slovak translation - *@dfridrich*

## [1.7.3](https://github.com/symfony/maker-bundle/releases/tag/1.7.3)

*March 4th, 2017*

### UNKNOWN

- [#94](https://github.com/knplabs/KnpTimeBundle/pull/94) - added Esperanto translation - *@marcu87*
- [#93](https://github.com/knplabs/KnpTimeBundle/pull/93) - Remove the ugly badge - *@wolfbyte18*

## [1.7.2](https://github.com/symfony/maker-bundle/releases/tag/1.7.2)

*February 2nd, 2017*

### UNKNOWN

- [#92](https://github.com/knplabs/KnpTimeBundle/pull/92) - Turkish translation bug fixed - *@shibby*

## [1.7.1](https://github.com/symfony/maker-bundle/releases/tag/1.7.1)

*January 20th, 2017*

### UNKNOWN

- [#91](https://github.com/knplabs/KnpTimeBundle/pull/91) - NewFeature: Catalan Translations - *@aleixfargas*

## [1.7.0](https://github.com/symfony/maker-bundle/releases/tag/1.7.0)

*October 22nd, 2016*

### UNKNOWN

- [#87](https://github.com/knplabs/KnpTimeBundle/pull/87) - Add DateTimeImmutable support - *@Einenlum*
- [#88](https://github.com/knplabs/KnpTimeBundle/pull/88) - Add Finnish language translations - *@nicodemuz*
- [#89](https://github.com/knplabs/KnpTimeBundle/pull/89) - Fix Polish Translations Pluralization - *@pawelabrams*
- [#84](https://github.com/knplabs/KnpTimeBundle/pull/84) - Remove useless note from the readme - *@stof*
- [#85](https://github.com/knplabs/KnpTimeBundle/pull/85) - Update README.md (yml + code) - *@Pierstoval*
- [#86](https://github.com/knplabs/KnpTimeBundle/pull/86) - Update .travis.yml - *@Pierstoval*

## [1.6.1](https://github.com/symfony/maker-bundle/releases/tag/1.6.1)

*May 10th, 2016*

### UNKNOWN

- [#82](https://github.com/knplabs/KnpTimeBundle/pull/82) - Added Kyrgyz translation - *@imanalopher*
- [#76](https://github.com/knplabs/KnpTimeBundle/pull/76) - add some Turkish related translations - *@behram*

## [1.6.0](https://github.com/symfony/maker-bundle/releases/tag/1.6.0)

*April 22nd, 2016*

### UNKNOWN

- [#83](https://github.com/knplabs/KnpTimeBundle/pull/83) - Adding a maintainers section - *@weaverryan*
- [#79](https://github.com/knplabs/KnpTimeBundle/pull/79) - Fix czech translation - *@dfridrich*
- [#80](https://github.com/knplabs/KnpTimeBundle/pull/80) - Fix the german translation - *@leofeyer*
- [#75](https://github.com/knplabs/KnpTimeBundle/pull/75) - add translation for vietnamese - *@DangChien*
- [#81](https://github.com/knplabs/KnpTimeBundle/pull/81) - Fix typo in time.uk.xliff - *@relo-san*

## [1.5.3](https://github.com/symfony/maker-bundle/releases/tag/1.5.3)

*November 24th, 2015*

## [1.5.2](https://github.com/symfony/maker-bundle/releases/tag/1.5.2)

*November 12th, 2015*

## [1.5.1](https://github.com/symfony/maker-bundle/releases/tag/1.5.1)

*November 11th, 2015*

## [1.5.0](https://github.com/symfony/maker-bundle/releases/tag/1.5.0)

*October 7th, 2015*

## [1.4.0](https://github.com/symfony/maker-bundle/releases/tag/1.4.0)

*September 17th, 2015*
